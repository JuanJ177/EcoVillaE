# app.py - EcoVillae listo para Vercel
from flask import Flask, request, jsonify, render_template, send_from_directory
from flask_cors import CORS
from db import get_connection
from datetime import datetime

# Flask buscará templates/ y static/ en la raíz del proyecto
...
# esta ruta realmente no es necesaria porque Flask ya sirve /static,
# pero si quieres mantenerla, que use app.static_folder:
...
def login():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, nombre FROM usuario WHERE email = %s AND contraseña = %s",
        (email, contraseña),
    )
    row = cur.fetchone()
    cur.close()
...
def buscar_usuario_api():
    ...
    return jsonify(
        [{"id": r[0], "nombre": r[1], "documento": r[2]} for r in rows]
    )
...
def usuario_detail(uid):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, nombre, email, telefono, numero_identificacion FROM usuario WHERE id = %s",
        (uid,),
    )
    r = cur.fetchone()
    ...
    cur.execute(
        "SELECT material, cantidad FROM reciclaje WHERE usuarioid = %s", (uid,)
    )
    rows = cur.fetchall()
    total_points, total_kg = 0, 0.0
    for mat, qty in rows:
...
def get_reciclaje():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, usuarioid, material, fecha, cantidad FROM reciclaje ORDER BY fecha DESC NULLS LAST"
    )
    rows = cur.fetchall()
    cur.close()
    conn.close()
...
def post_reciclaje():
    ...
    if not usuarioid or not material or cantidad is None:
        return jsonify(
            {"error": "usuarioid, material y cantidad requeridos"}
        ), 400
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO reciclaje (usuarioid, material, fecha, cantidad) VALUES (%s, %s, %s, %s) RETURNING id",
...
def get_ranking():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT usuarioid, material, SUM(cantidad) FROM reciclaje GROUP BY usuarioid, material"
    )
    rows = cur.fetchall()
    agg = {}
    for uid, mat, s in rows:
        agg[uid] = agg.get(uid, 0) + compute_points(mat, s)
    ...
    data = [
        {"usuarioid": uid, "nombre": users.get(uid, f"Usuario {uid}"), "points": pts}
